...
let restaurantImagePaths = importAll(
  require.context('../assets/restaurantsImages/', false, /\.(jpg)$/)
);
let resortImagePaths = importAll(require.context('../assets/resortsImages/', false, /\.(png)$/));
// false, /\.(png|jpe?g|svg)$/));

const pathStringKey = 'static/media/';
restaurantImagePaths = restaurantImagePaths
  .map(e => e.default)
  .filter(e => e.includes(pathStringKey));
...
const restaurantsArc1600 = restaurantsData
  ? restaurantsData.filter(e => e.resort == 'arc1600')[0].restaurants.filter(e => e.enabled)
  : {};
const restaurantsArc1800 = restaurantsData
  ? restaurantsData.filter(e => e.resort == 'arc1800')[0].restaurants.filter(e => e.enabled)
  : {};
const restaurantsArc1950 = restaurantsData
  ? restaurantsData.filter(e => e.resort == 'arc1950')[0].restaurants.filter(e => e.enabled)
  : {};
const restaurantsArc2000 = restaurantsData
  ? restaurantsData.filter(e => e.resort == 'arc2000')[0].restaurants.filter(e => e.enabled)
  : {};
...
const resorts = resortsData ? resortsData.filter(e => e.enabled) : {};
