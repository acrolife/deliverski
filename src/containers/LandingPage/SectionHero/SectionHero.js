...
const restaurantsMailtoMarmott = (
  <a
    href={`mailto:${emailAddress}?subject=${emailSubject}&body=${messageContent}`}
    className={css.heroButtonPromo}
  >
  ...
  return (
    <div className={classes} style={divStyle}>
      <div className={css.heroContent}>
        ...
        {isProduction && (
          <h3 className={css.messageCheckPlaygroundMobile}>
            {/* <FormattedMessage id="SectionHero.messageCheckPlaygroundWithLink" values={{ lineBreak: <br />, playgroundLink }} /> */}
            <FormattedMessage
              id="SectionHero.messageCheckPlaygroundMobile"
              values={{ lineBreak: <br /> }}
            />
          </h3>
        )}
        ...
        <h1 className={isProduction ? css.heroMainTitlePromo : css.heroMainTitle}>
          {isProduction ? (
            <FormattedMessage id="SectionHero.titleRestaurants" />
          ) : (
        ...
        {!isProduction && (
          <h2 className={isProduction ? css.heroSubTitlePromo : css.heroSubTitle}>
            {isProduction ? (
              <FormattedMessage id="SectionHero.subTitleRestaurants" />
            ) : (
