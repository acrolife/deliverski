...
const integrationSdk = sharetribeIntegrationSdk.createInstance({
  ...
  module.exports = (req, response) => {
    ...
    const promises = restOfShoppingCartItems.map(x => {
        return integrationSdk.stockAdjustments.create({
                    listingId: x.listing.id.uuid,
                    quantity: Number(x.listing.currentStock.attributes.quantity) - Number(x.checkoutValues.quantity)//TODO see if correct
